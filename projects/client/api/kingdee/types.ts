...
export interface BillQueryParam {
  FormId: string;
  TopRowCount?: number; // 最多允许查询的数量，0或不传表示不限制
  Limit?: number; // 分页取数每页允许获取的数据，最大不能超过2000（可调整到10000）
  StartRow?: number; // 分页取数开始行索引，从0开始
  FilterString?: string; // 过滤条件，一定要有查询条件并限定取数范围
  OrderString?: string; // 排序条件
  FieldKeys: string; // 逗号分隔字段标识，基础资料字段不要超过30个
  SubSystemId?: string; // 子系统Id，部分查询接口（如 BillQuery）需要
}
...
export interface SaveParam<T = unknown> {
  ...
  ValidateFlag?: string; // "true" / "false"
  NumberSearch?: string; // "true" / "false"
  IsAutoAdjustField?: string; // "true" / "false"
  IsControlPrecision?: string; // "true" / "false"
  ValidateRepeatJson?: string; // "true" / "false"
  Model: T;
